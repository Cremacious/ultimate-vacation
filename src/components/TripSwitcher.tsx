...
export default function TripSwitcher({ currentTrip }: TripSwitcherProps) {
  ...
  useEffect(() => {
    ...
    if (loadState === "idle") {
      ...
      fetchUserTripsAction()
        .then((data) => { setTrips(data); setLoadState("done"); })
    ...
    <div className="relative">
      ...
      {open && (
        <div
          ...
          tabIndex={-1}
          className="absolute right-0 top-full mt-2 w-72 max-w-[calc(100vw-24px)] rounded-2xl border border-[#2A2B45] focus:outline-none z-[100]"
          style={{ backgroundColor: "#15162A", boxShadow: "0 8px 32px rgba(0,0,0,0.5)" }}
        >
          {/* Current trip row */}
          <div className="px-4 py-3 flex items-center gap-2.5">
            <span
            ...
            <span className="text-xs bg-white/5 text-white/30 px-2 py-0.5 rounded-full flex-shrink-0">
              current
            </span>
          </div>
          ...
          {loadState === "loading" && (
            <>
              <div className="border-t border-[#2A2B45] mx-4" />
              <SkeletonRow />
              <SkeletonRow />
            </>
          )}
